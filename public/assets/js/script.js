...
!function (a, b) {
    ...
    l.cors = !!Ib && "withCredentials" in Ib, l.ajax = Ib = !!Ib, n.ajaxTransport(function (b) {
        ...
        if (a.crossDomain) {
            ...
            return {
                send: function (e, f) {
                    b = n("<script>").prop({charset: a.scriptCharset, src: a.url}).on("load", c = function (a) {
                        b.remove(), c = null, a && f("error" === a.type ? 404 : 200, a.type)
                    }), d.head.appendChild(b[0]).on("error", c = function (a) {
                        b.remove(), c = null, a && f("error" === a.type ? 404 : 200, a.type)
...
$(document).ready(function () {
    ...
    var token = "";
    var status = "";
    var comment = first[0].replace("comment-", "");
    if (comment.length > 0) {
        console.log(comment);
        // var request = $.ajax({
        //     url: "/comment",
...
$(document).on('mouseover', ".date", function () {
    ...
    if (Object.prototype.toString.call(timedate) === "[object Date]") {
        if (isNaN(timedate.getTime())) {
            var timedate = new Date($(this).text() * 1000);
            if (Object.prototype.toString.call(timedate) === "[object Date]") {
                if (isNaN(timedate.getTime())) {
                }
                else {
                    $(this).text(timedate.toLocaleString());
                }
            }
            else {
                var timedate = new Date($(this).text() * 1000);
...
var track_page = 3; //track user scroll as page number, right now page number is 1
...
function load_contents(track_page) {
    if (loading == false) {
        ...
        $.post('/api/fetchpage', {'article': track_page}, function (data) {
            loading = false; //set loading flag off once the content is loaded
            if (data.trim().length == 0) {
                //notify user if nothing to load
                $('.loader').html('<p>No more articles!</p>');
                return;
            }
            $('.loader').hide(); //hide loading animation once data is received
            $(".post-list").append(data); //append data into #results element
...
!function (e) {
    ...
    function u(e, r, n, i) {
        ...
        function p(e, t, r, a) {
            var n = a ? "" : R.classPrefix, i = '<span class="' + n, s = r ? "" : B;
            return i += e + '">', i + t + s
        }
    ...
    var w = [], N = Object.keys, k = {}, x = {}, C = /^(no-?highlight|plain|text)$/i, E = /\blang(?:uage)?-([\w-]+)\b/i, M = /((^(<[^>]+>|\t|)+|(?:\n)))/gm, B = "</span>", R = {
        classPrefix: "hljs-",
        tabReplace: null,
        useBR: !1,
        languages: void 0
    ...
    return e.highlight = u, e.highlightAuto = d, e.fixMarkup = b, e.highlightBlock = m, e.configure = f, e.initHighlighting = g, e.initHighlightingOnLoad = _, e.registerLanguage = h, e.listLanguages = v, e.getLanguage = y, e.inherit = s, e.IR = "[a-zA-Z]\\w*", e.UIR = "[a-zA-Z_]\\w*", e.NR = "\\b\\d+(\\.\\d+)?", e.CNR = "(-?)(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?|\\.\\d+)([eE][-+]?\\d+)?)", e.BNR = "\\b(0b[01]+)", e.RSR = "!|!=|!==|%|%=|&|&&|&=|\\*|\\*=|\\+|\\+=|,|-|-=|/=|/|:|;|<<|<<=|<=|<|===|==|=|>>>=|>>=|>=|>>>|>>|>|\\?|\\[|\\{|\\(|\\^|\\^=|\\||\\|=|\\|\\||~", e.BE = {
        ...
        var t = {
            ...
            i: /\n/,
            c: [{b: "{{"}, {b: "}}"}, e.BE, i]
        }, c = {cN: "string", b: /\$@"/, e: '"', c: [{b: "{{"}, {b: "}}"}, {b: '""'}, n]}, o = e.inherit(c, {
            i: /\n/,
            c: [{b: "{{"}, {b: "}}"}, {b: '""'}, i]
        });
        ...
        var t = "[a-zA-Z_]\\w*[!?=]?|[-+~]\\@|<<|>>|=~|===?|<=>|[<>]=?|\\*\\*|[-/+%^&*~`|]|\\[\\]=?", r = {
            ...
            cN: "function", bK: "def",
            e: "$|;", c: [e.inherit(e.TM, {b: t}), o].concat(i)
